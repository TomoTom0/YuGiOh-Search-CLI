...
# Download faq-all.tsv
echo "Downloading faq-all.tsv..."
HTTP_CODE=$(curl -L -w "%{http_code}" -o "$DATA_DIR/faq-all.tsv" -sS \
  "$RELEASE_URL/$VERSION/faq-all.tsv" 2>&1 | tail -n 1)

if [ "$HTTP_CODE" != "200" ]; then
  echo "Error: Failed to download faq-all.tsv (HTTP $HTTP_CODE)"
  echo "URL: $RELEASE_URL/$VERSION/faq-all.tsv"
  rm -f "$DATA_DIR/faq-all.tsv"
  exit 1
fi
...
FAQ_SIZE=$(stat -f%z "$DATA_DIR/faq-all.tsv" 2>/dev/null || stat -c%s "$DATA_DIR/faq-all.tsv" 2>/dev/null)

if [ "$CARDS_SIZE" -lt 1000000 ] || [ "$DETAIL_SIZE" -lt 1000000 ] || [ "$FAQ_SIZE" -lt 1000000 ]; then
  ...
  echo "faq-all.tsv: $FAQ_SIZE bytes"
  exit 1
fi
...
echo "  - $DATA_DIR/faq-all.tsv ($(numfmt --to=iec-i --suffix=B $FAQ_SIZE 2>/dev/null || echo "$FAQ_SIZE bytes"))"
echo ""
