...
/**
 * 残りのFAQデータをTSVからD1にインポートするスクリプト
 *
 * 使い方:
 *   tsx scripts/import-remaining-faqs.ts <remaining-faq-ids.txt> <faq-all.tsv>
 */
...
async function main() {
  ...
  if (args.length < 2) {
    console.error('Usage: tsx scripts/import-remaining-faqs.ts <remaining-faq-ids.txt> <faq-all.tsv>');
    process.exit(1);
  }
  ...
  const outputFile = 'import-remaining-faqs.sql';
