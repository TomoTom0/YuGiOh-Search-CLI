...
impl CardSearcher {
    ...
    ///
    /// 照合先は `card.name_modified`。これはTSの `nameModified`（= DBの
    /// `normalized_name`、`normalizeForSearch(name)` 相当）と一致する。
    pub fn search_by_name<'a>(cards: &'a [Card], query: &str) -> Vec<&'a Card> {
        ...
        if normalized_query.contains('*') {
            ...
            for card in cards {
                if re.is_match(&card.name_modified) {
        ...
        // 通常の検索
        for card in cards {
            if card.name_modified == normalized_query {
                exact_matches.push(card);
            } else if card.name_modified.contains(&normalized_query) {
                fuzzy_matches.push(card);
            }
        }
...
mod tests {
    ...
    /// テスト用カードを生成する。検索対象（`name`/`name_modified`/`card_id`）のみ実値を
    /// 与え、モンスター固有などの残フィールドは空または `None` とする。
    /// `name_modified` はTSの `nameModified`（`normalizeForSearch(name)` 相当）。
    fn mk_card(card_id: &str, name: &str, name_modified: &str) -> Card {
        Card {
    ...
    fn mock_cards() -> Vec<Card> {
        vec![
            mk_card("4007", "青眼の白龍", "青眼ノ白龍"),
            mk_card("10000", "青眼の亜白龍", "青眼ノ亜白龍"),
            mk_card("2129", "青眼の究極竜", "青眼ノ究極龍"),
            mk_card("5678", "ブラック・マジシャン", "ブラックマジシャン"),
            mk_card(
                "9012",
                "E・HERO フレイム・ウィングマン",
                "eheroフレイムウィングマン",
    ...
    fn test_search_prioritizes_exact_match() {
        ...
        let cards = vec![
            mk_card("1", "青眼", "青眼"),
            mk_card("2", "青眼の白龍", "青眼ノ白龍"),
            mk_card("3", "青眼の亜白龍", "青眼ノ亜白龍"),
        ];
