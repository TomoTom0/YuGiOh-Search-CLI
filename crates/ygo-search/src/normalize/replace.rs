...
impl PatternReplacer {
    ...
    pub fn replace(
        ...
        let mut pairs: Vec<_> = patterns.into_iter().zip(search_results).collect();
            ...
            match &pattern.pattern_type {
                ...
                _ => {
                    ...
                    if results.len() == 1 {
                        ...
                        let replacement =
                            format!("{{{{`{}`_{}}}}}", pattern.query, candidates.join("_"));
            ...
            if let PatternType::CardId = &pattern.pattern_type {
                if results.is_empty() {
                    ...
                    warnings.push(format!(
                        "cardId \"{}\" found multiple cards. Please check data.",
                        pattern.query
                    ));
                }
            }
        ...
        if has_unprocessed {
            ...
            if notfound_count > 0 {
                warnings.push(format!(
                    "Found {} pattern(s) with no matches (NOTFOUND_*)",
                    notfound_count
                ));
            }
            if multiple_count > 0 {
                warnings.push(format!(
                    "Found {} pattern(s) with multiple matches - please select correct one",
                    multiple_count
                ));
            }
        }
...
mod tests {
    ...
    fn test_replace_single_match_resolved() {
        ...
        assert_eq!(
            result.processed_patterns[0].status,
    ...
    fn test_replace_not_found() {
        ...
        assert_eq!(
            ...
            "Use {{NOTFOUND_`NonExistentCard12345`}} here"
        );
        assert!(result.has_unprocessed);
    ...
    fn test_replace_corrected_card_name() {
        ...
        assert!(result
            .warnings
            .iter()
            .any(|w| w.contains("Card name corrected")));
        assert!(result
            .warnings
            .iter()
    ...
    fn test_replace_mixed_patterns() {
        ...
        let already_proc = result
            .processed_patterns
            .iter()
            .find(|p| p.status == ReplacementStatus::AlreadyProcessed);
        ...
        let resolved = result
            .processed_patterns
            .iter()
            .find(|p| p.status == ReplacementStatus::Resolved);
        ...
        let multiple = result
            .processed_patterns
            .iter()
            .find(|p| p.status == ReplacementStatus::Multiple);
    ...
    fn test_replace_warnings_for_multiple_and_notfound() {
        ...
        assert!(result
            .warnings
            .iter()
            .any(|w| w.contains("multiple matches")));
    }
}
